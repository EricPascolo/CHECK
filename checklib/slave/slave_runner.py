...
def worker(check_core):
    ...
    # get scheduler env
    scheduler = whatscheduler.check_installed_scheduler(check_core.setting)

    if not scheduler:
        from socket import gethostname, gethostbyaddr

        if gethostname().find('.') >= 0:
            nameofnode = gethostname()
        else:
            nameofnode = gethostbyaddr(gethostname())[0]
    ...
    with open(check_core.setting["resultfile"], "a+") as out_file:
