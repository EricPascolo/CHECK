...
def resolve_env_path(dictionary):
    ...
    for key, value in get_iter_object_from_dictionary(dictionary):
        # TODO: investigate on this try. When it is removed a boolean is passed to the function.
        try:
...
def get_setting_file_path(filename):
    """ Return a list of setting file given its name, the search path is in order etc and etc/default """

    check_setting_path = []
    check_setting_found = False
    ...
    if not check_setting_found:
        logger.critical("ERROR CHECK setting file not found:" + filename)
        raise FileNotFoundError("Could not find settings file")
...
def get_iter_object_from_dictionary(d):
    """ Return different iter object of dictionary, the objects depend on python's version"""
